                                ...
                                @foreach(config("i18n.locales") as $key => $locale)
                                    ...
                                    <div id="tab-{{ $key }}"
                                        ...
                                        <div class="form-group">
                                            <label for="input-title-{{ $key }}">{{ trans("pages::pages.attributes.title", [], $key) }}</label>
                                            <textarea name="title[{{$key}}]" class="form-control input-lg" rows="1"
                                                      id="page_title_[{{$key}}]"
                                                      placeholder="{{ trans("pages::pages.attributes.title") }}">{{ @Request::old("title", $page->title->$key) }}</textarea>
                                        </div>

                                        <div class="form-group">
                                            <label for="input-slug-{{ $key }}">{{ trans("pages::pages.attributes.slug", [], $key) }}</label>
                                            <textarea name="slug[{{$key}}]" class="form-control input-lg" rows="1"
                                                      id="page_slug_[{{$key}}]"
                                                      placeholder="{{ trans("pages::pages.attributes.slug") }}">{{ @Request::old("slug", $page->slug->$key) }}</textarea>
                                        </div>

                                        <div class="form-group">
                                            <label for="input-pages-{{ $key }}">{{ trans("pages::pages.attributes.excerpt", [], $key) }}</label>
                                            <textarea name="excerpt[{{$key}}]" class="form-control"
                                                      id="page_excerpt_[{{$key}}]"
                                                      placeholder="{{ trans("pages::pages.attributes.excerpt") }}">{{ @Request::old("excerpt", $page->excerpt->$key) }}</textarea>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            @include("posts::editor", [
                                                            ...
                                                            "placeholder" => trans("pages::pages.attributes.content", [], $key),
                                                            "value" => @Request::old("content." . $key, $page->content->$key),
                                                            "name" => 'content['. $key .']'
                                                        ])
                                        </div>
                                    </div>
