        ...
        <div class="row wrapper border-bottom white-bg page-heading">
            <div class="col-lg-4 col-md-6 col-sm-6 col-xs-12">
                <h2>
                    ...
                    {{ $industry ? trans("industries::industries.edit") : trans("industries::industries.add_new") }}
                </h2>
                <ol class="breadcrumb">
                    <li>
                    ...
                    <li>
                        <a href="{{ route("admin.industries.show") }}">{{ trans("industries::industries.industries") }}</a>
                    </li>
                    <li class="active">
                        <strong>
                            {{ $industry ? trans("industries::industries.edit") : trans("industries::industries.add_new") }}
                ...
                @if ($industry)
                    <a href="{{ route("admin.industries.create") }}"
                       class="btn btn-primary btn-labeled btn-main"> <span
                        ...
                        {{ trans("industries::industries.add_new") }}</a>
                @endif

                <button type="submit" class="btn btn-flat btn-danger btn-main">
                    <i class="fa fa-download" aria-hidden="true"></i>
                    {{ trans("industries::industries.save_industry") }}
                </button>
                    ...
                    <div class="panel panel-default">
                        <div class="panel-body">
                            ...
                            <div class="form-group">
                                <label for="input-name">{{ trans("industries::industries.attributes.name") }}</label>
                                <input name="name" type="text"
                                       value="{{ @Request::old("name", $industry->name) }}"
                                       class="form-control" id="input-name"
                                       placeholder="{{ trans("industries::industries.attributes.name") }}">
                            </div>

                            <div class="form-group">
                                <label for="input-name">{{ trans("industries::industries.attributes.logo") }}</label>
                                <input name="logo" type="text"
                                       value="{{ @Request::old("logo", $industry->logo) }}"
                                       class="form-control" id="input-name"
                                       placeholder="{{ trans("industries::industries.attributes.logo") }}">
                            </div>

                        </div>
                    </div>

                    @foreach(Action::fire("industry.form.featured") as $output)
                        {!! $output !!}
                    @endforeach

                </div>
                <div class="col-md-4">
                    @foreach(Action::fire("industry.form.sidebar") as $output)
                        {!! $output !!}
                    @endforeach
