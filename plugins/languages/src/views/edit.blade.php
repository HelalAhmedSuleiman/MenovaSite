        ...
        <div class="row wrapper border-bottom white-bg page-heading">
            <div class="col-lg-4 col-md-6 col-sm-6 col-xs-12">
                <h2>
                    ...
                    {{ $language ? trans("languages::languages.edit") : trans("languages::languages.add_new") }}
                </h2>
                <ol class="breadcrumb">
                    <li>
                    ...
                    <li>
                        <a href="{{ route("admin.languages.show") }}">{{ trans("languages::languages.languages") }}</a>
                    </li>
                    <li class="active">
                        <strong>
                            {{ $language ? trans("languages::languages.edit") : trans("languages::languages.add_new") }}
                ...
                @if ($language)
                    <a href="{{ route("admin.languages.create") }}"
                       class="btn btn-primary btn-labeled btn-main"> <span
                        ...
                        {{ trans("languages::languages.add_new") }}</a>
                @endif

                <button type="submit" class="btn btn-flat btn-danger btn-main">
                    <i class="fa fa-download" aria-hidden="true"></i>
                    {{ trans("languages::languages.save_language") }}
                </button>
                    ...
                    <div class="panel panel-default">
                        <div class="panel-body">
                            ...
                            <div class="form-group">
                                <label for="input-name">{{ trans("languages::languages.attributes.name") }}</label>
                                <input name="name" type="text"
                                       value="{{ @Request::old("name", $language->name) }}"
                                       class="form-control" id="input-name"
                                       placeholder="{{ trans("languages::languages.attributes.name") }}">
                            </div>

                        </div>
                    </div>

                    @foreach(Action::fire("language.form.featured") as $output)
                        {!! $output !!}
                    @endforeach

                </div>
                <div class="col-md-4">
                    @foreach(Action::fire("language.form.sidebar") as $output)
                        {!! $output !!}
                    @endforeach
